...
# xxx (insignificant for physiology)
#gene <- c("")
...
# drop condition
drops <- grep('_FW_', colnames(cts), value=TRUE)
#drops <- grep('_BW_', colnames(cts), value=TRUE)
#drops <- grep('_transfer_', colnames(cts), value=TRUE)

# run PGLS 
cts_condition <- cts[ , !(names(cts) %in% drops)]
