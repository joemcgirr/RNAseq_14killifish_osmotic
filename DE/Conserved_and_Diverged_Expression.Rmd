...
### Conserved across all species: Genes show the same response to osmotic stress across all species, regardless of native physiology. 
  
  - Same fold-change expression response in all species.
...
### Diverged expression between euryhaline and steonhaline physiologies that is conserved across clades.  

  - Opposite fold-change expression responses with native physiology.
...
### Diverged expression between euryhaline and steonhaline physiologies that is diverged across clades. 

  - Opposite fold-change expression responses with native physiology dependent on clade.
...
### Example genes: exoc7 and fbxo28
exoc7 and fbxo28 show divergent expression patterns between physiology depending on clade
...
### Heatmap: Genes showing significant treatment x physiology 
  - These genes show the same type of expression pattern as exoc7 and fbxo28. 
  - log2(mean.species.BW) - log2(mean.species.FW)
  - Genes with large fold changes (> 2 or < -2) have been filtered out.
...
### Heatmap: Genes showing significant BW response after 24 hours
...
## Top 500 genes showing significant treatment x physiology using PGLS approach
...
pgls_sig_genes <- pgls_sig_genes[pgls_sig_genes$p < 0.001,]
...
hm <- heatmap_data_munge(pgls_gene_ids_acclimation,experiment)
...
hm <- heatmap_data_munge(pgls_gene_ids_transfer,experiment)
#hm <- heatmap_data_munge_strict_count_filt(gene_ids,experiment)
